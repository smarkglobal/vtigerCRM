...
$(document).ready(function() {
	/*$("select[name='productcategory']").on('change', function(){
		if ($("select[name='productcategory']").chosen().val() == '吸毒評估') {
			alert('hello');
			$('th.blockHeader:contains("戒酒輔導服務")').closest("table").hide();
			alert('hello');
		}
	});*/
	if ( $("#Products_detailView_fieldValue_productcategory .value").is(':contains("吸毒評估")') ) {
		$('th.blockHeader:contains("戒酒輔導服務")').closest("table").hide();
	}
});
