...
class Contacts extends CRMEntity {
	...
	var $related_module_table_index = array(
		...
		'TestingModule' => array('table_name' => 'vtiger_testingmodule', 'table_index' => 'crmid', 'rel_index' => 'testingmoduleid'),
	);
...
// vtiger_account query goes for ecery single vtiger_account row
	...
	*/
	...
	function get_testingmodules($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		global $log, $singlepane_view,$currentModule,$current_user;
		$log->debug("Entering get_testingmodules(".$id.") method ...");
		$this_module = $currentModule;
		...
		$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
							'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
		$query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
				vtiger_crmentity.crmid, vtiger_testingmodule.*
				from vtiger_testingmodule inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_testingmodule.testingmoduleid
				LEFT JOIN vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
				LEFT JOIN vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
				WHERE vtiger_crmentity.deleted=0 and vtiger_testingmodule.contactid=".$id;
		...
		$log->debug("Exiting get_testingmodules method ...");
		return $return_value;
	}
